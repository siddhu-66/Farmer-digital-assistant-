  ...
  const renderInput = (name: keyof FormData, placeholder: string, type: string = 'text', required: boolean = false) => (
    <div>
      <input
        ...
        className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 ${errors[name] ? 'border-red-500 focus:ring-red-500' : 'focus:ring-green-500'}`}
        required={required}
      />
      ...
      {step === 'otp' && (
        <form onSubmit={handleOtpSubmit}>
          ...
          <input
            ...
            className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 ${errors.otp ? 'border-red-500 focus:ring-red-500' : 'focus:ring-green-500'}`}
            required
          />
